...
\- Environment staging: Earth/Moon/Ocean positions+scales+opacity and the key light (sun) position are per-phase data in SETTLED\[i].env, lerped through the same continuous channel as the rocket transform (\_applyContinuous). The Ocean (environment/Ocean.js, splashdown water) follows the same apply() contract and stays at the splash site with opacity 0 until phase 13; its disc carries renderOrder -1 like the pad, for the same transparent-pass reason. The Moon holds at its phase-7 entry mark (opacity 0) all through ascent and fades in only at TLI, so ITS reveal is a fade not a sweep. Phase 9's Moon is sized/placed so the sphere's TOP surface sits exactly under the LM footpads at the settled pos — the Moon sphere IS the landing terrain (no separate patch; a tiled procedural crater-field bump map carries close-range detail). The 8->9 moon lerp path was chosen to keep that surface below the descending vehicle the whole way — don't move those env numbers without re-checking clearance.

\- Earth is a CONTINUOUS pull-back from pad to orbit, not a phase-7 pop: ENV\_ASCENT\[3..6] reveal the Earth globe during ascent as a huge curved backdrop the vehicle climbs away from (Max-Q fills the frame; by TLI it's a small arc at the bottom), leading straight into SETTLED\[7]'s marble. The ascent cameras look UP the stack, so each Earth center is placed below-and-behind that shot's view axis (~45° pitched down, distance growing each phase) and solved against the SETTLED camera of that phase — nudging a number means re-checking the camera stays OUTSIDE the sphere (radius 1800) through the beat, or the front-face-culled globe vanishes (beatmid clearance check). Phases 0-2 hold Earth at the far ENV\_HOME mark (opacity 0) so the 2->3 beat fades+swells it in. Gotcha: the flat launch-pad terrain would read as a translucent shelf across the curved globe during that crossfade, so the pad channel alone is front-loaded in \_applyContinuous (`padT = min(1, easeT/0.4)`) to vanish before the Earth swells; phase-3 pad target is 0. Earth texture anisotropy is 16 (grazing-angle ascent views).
